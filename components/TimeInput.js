...
export default class TimeInput extends Component {
    ...
    state = {
        date: this.formatDate(new Date(), 'd MMM yyyy'),
    ...
    componentDidMount() {
        ...
        setInterval(() => {
            const date = new Date();
            this.setState({ date: this.formatDate(date, 'd MMM yyyy') })
    ...
    async uploadDateTime() {
        ...
        const timeURL = baseURL +
        '?date_time=' + dateTime
        ToastAndroid.show(timeURL, ToastAndroid.SHORT)
        
        await fetch(timeURL, {
            method: 'GET',
        }
        ).then((response) => response.text())
            .then((responseData) => { ToastAndroid.show(responseData, ToastAndroid.SHORT) })
            .catch((error) => {
